    ...
    public class CertificateFromFileOperation : RemoteCompositeOperation
    {
        ...
        public override void Configure(IOfferRemoteComposition server)
        {
            ...
            if(cert.HasPrivateKey)
            {
                ...
                server.ExecuteRemote.PowerShell("$path=\"" + destPath + "\"; $password='" + _password + "'; $privateKeyUsers = " + psUserArray + "; [ConDep.Dsl.Remote.Resources.CertificateInstaller]::InstallPfx($path, $password, $privateKeyUsers);", opt => opt.RequireRemoteLib());
            }
            else
            {
